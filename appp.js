document.getElementById('recommandForm').addEventListener('submit', function(event) {
  ...
  let fertilisers='Phosphatic and pottasic fertilizers'
  if (ph >= 6.0 && ph <= 7.5 && moisture >= 30 && moisture <= 60 && temperature >= 10 && temperature <= 30 && organicContent >= 2) {
      soilQuality = "Good";
      fertilisers='10-10-10 NPK and 15-15-15 NPK'
  ...
  // Display result
  const Result = document.getElementById('recommandedResult');
  const RecommandElement = document.getElementById('recommand');
  
  
  RecommandElement.textContent = `The recommanded fertilizers are : ${fertilisers}  and more `;
